...
if ( class_exists('\\MNT\\UI\\Init') ) {
	$taskbot_order_completed = count( \MNT\UI\Init::mnt_get_seller_task_orders( $user_identity, 'completed' ) );
	$taskbot_order_hired      = count( \MNT\UI\Init::mnt_get_seller_task_orders( $user_identity, 'hired' ) );
} else {
	// Legacy fallback when helper isn't available
	$meta_array	= array(
		array(
			'key'		=> 'seller_id',
			'value'		=> $user_identity,
	...
	$taskbot_order_completed  = taskbot_get_post_count_by_meta('shop_order','any',$meta_array);
	$meta_array	= array(
		array(
			'key'		=> 'seller_id',
			'value'		=> $user_identity,
	...
	$taskbot_order_hired    = taskbot_get_post_count_by_meta('shop_order','any',$meta_array);
}
