...
<div class="tb-dhb-profile-settings">
	...
	<?php 
	// Check if user has a wallet and display wallet section OUTSIDE the billing form
	$user_has_wallet = false;
	if (function_exists('mnt_user_has_wallet')) {
		$user_has_wallet = mnt_user_has_wallet($user_identity);
	} elseif (shortcode_exists('mnt_wallet_dashboard')) {
		// Alternative check - if wallet meta exists
		$wallet_id = get_user_meta($user_identity, 'mnt_wallet_id', true);
		$user_has_wallet = !empty($wallet_id);
	}
	
	// Display appropriate shortcode based on wallet status
	if ($user_has_wallet && shortcode_exists('mnt_wallet_dashboard')) {
		// User has wallet - show dashboard
		echo do_shortcode('[mnt_wallet_dashboard]');
	} elseif (shortcode_exists('mnt_create_wallet')) {
		// User doesn't have wallet - show create wallet
