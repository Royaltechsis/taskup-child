...
$meta_array	= array(
 ...
 $completed_tasks  = taskbot_tasks_earnings('shop_order','any',$meta_array);
...
//Gigs Area
// Aggregate seller task counters using centralized helper to avoid WP_Query meta JOIN edge-cases
$gig_completed = 0;
$gig_queues = 0;
$gig_cancelled = 0;
if ( class_exists('\\MNT\\UI\\Init') ) {
    $gig_completed = count( \MNT\UI\Init::mnt_get_seller_task_orders( $user_identity, 'completed' ) );
    $gig_queues    = count( \MNT\UI\Init::mnt_get_seller_task_orders( $user_identity, 'hired' ) );
    $gig_cancelled = count( \MNT\UI\Init::mnt_get_seller_task_orders( $user_identity, 'cancelled' ) );
} else {
    // Legacy per-product aggregation fallback
    ...
    if ( $taskbot_gigs_query->have_posts() ){
        while ( $taskbot_gigs_query->have_posts() ){
            ...
            $taskbot_task_queues = taskbot_get_post_count_by_meta('shop_order', 'any', $meta_task_queues_array);
            ...
            $taskbot_task_completed = taskbot_get_post_count_by_meta('shop_order', 'any', $meta_task_completed_array);
            ...
            $taskbot_task_cancelled = taskbot_get_post_count_by_meta('shop_order', 'any', $meta_task_cancelled_array);
