 ...
 * Child theme override to use MNT Wallet balance
 *
 * @package     Taskbot
...
// Use MNT Wallet balance instead of Taskbot balance
$available_withdraw_amount = 0;
if (class_exists('MNT\Api\wallet')) {
    $wallet_result = \MNT\Api\wallet::balance($user_identity);
    $available_withdraw_amount = $wallet_result['balance'] ?? 0;
} else {
    // Fallback to original Taskbot calculation
